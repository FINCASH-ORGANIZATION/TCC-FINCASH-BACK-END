...
export const criarConta = async (req, res) => {
    try {
        ...
        const bancosValidos = ['Banco do Brasil', 'Caixa', 'Itau', 'Santander', 'Nubank', 'Bradesco', 'Inter'];
        if (!bancosValidos.includes(banco)) {
            return res.status(400).send({ mensagem: 'Banco inválido! Escolha entre: Banco do Brasil, Caixa, Itau, Santander, Nubank, Bradesco e Inter.' });
        }
        ...
        res.status(201).send({ mensagem: 'Conta do banco criada com sucesso!', conta: novaConta });
    } catch (error) {
