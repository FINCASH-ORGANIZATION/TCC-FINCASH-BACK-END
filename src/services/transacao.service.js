...
export const pesIDService = (id) =>
    transacao.findById(id)
        .populate('categoria')
        .populate('Usuario');

export const pesqDescricaoService = (descricao) =>
    transacao.find({
...
export const pesUsuarioService = (id) =>
    transacao.find({ Usuario: id })
        .sort({ _id: -1 })
        .populate('categoria')
        .populate('Usuario');

export const atualizarTransService = (id, valor, data, descricao, tipoTransacao, categoria, formaPagamento, conta, categoriaPersonalizada) =>
    transacao.findOneAndUpdate(
        { _id: id },
        { valor, data, descricao, tipoTransacao, categoria, formaPagamento, conta, categoriaPersonalizada },
        { rawResult: true }
    );
